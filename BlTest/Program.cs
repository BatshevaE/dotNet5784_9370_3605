...
internal class Program
{
    ...
    public enum TaskSubMenue

    {
        Exit, Creat, Read, ReadAll, Update, Delete, updateStartDate,
    }
    public enum EngSubMenue

    {
        Exit, Creat, Read, ReadAll, Update, Delete, CalculateTaskInEngineer,
    }
...
Delete:5
 ");
                if (!TaskSubMenue.TryParse(Console.ReadLine(), out choiceTask)) //read the int choice and convert it to SubMenue types
                    throw new FormatException("wrong input");
                switch (choiceTask)
                {
                    case TaskSubMenue.Exit:
                        return;
                    case TaskSubMenue.Creat:
                        creatTask();
                        break;
                    case TaskSubMenue.Read:
                        readTask();
                        break;
                    case TaskSubMenue.ReadAll:
                        readListTasks();
                        break;
                    case TaskSubMenue.Update:
                        updateTask();
                        break;
                    case TaskSubMenue.Delete:
                        deleteTask();
                        break;
                    case TaskSubMenue.updateStartDate:
                        updateStartDate();
                        break;
                    default:
                        return;
...
Update:4
Delete:5");
                if (!EngSubMenue.TryParse(Console.ReadLine(), out choiceEngineer)) //read the int choice and convert it to SubMenue types
                    throw new FormatException("wrong input");

                switch (choiceEngineer)
                {
                    case EngSubMenue.Exit:
                        return;
                    case EngSubMenue.Creat:
                        createEngineer();
                        break;
                    case EngSubMenue.Read:
                        readEngineer();
                        break;
                    case EngSubMenue.ReadAll:
                        readAllEngineers();
                        break;
                    case EngSubMenue.Update:
                        updateEngineer();
                        break;
                    case EngSubMenue.Delete:
                        deleteEngineer();
                        break;
                    case EngSubMenue.CalculateTaskInEngineer:
                        CalculateTaskInEngineer();
                        break;
                    default:
                        return;
                }
 ...
 Name:");
        ...
        BO.Task? task = new(taskDescriptoin);
