...
internal class TaskImplemenation : ITask
{
    ...
    public void Delete(int id)
    {
        ...
        IEnumerable<DO.Dependency>? taskDependent=from dependent in dependentList//a collection of all the dependecies with the tasks that depencence on this task
                                                  ...
                                                  select dependent;
        if (taskDependent != null)//the collection is not null-there are tasks the dependence on this task
    ...
    public void Update(BO.Task item)
    {
        ...
        try
        {
            ...
            {
                if (FindDependent(item.Id, dependency.Id) != 0)
                {
                    _dal.Dependency.Update(new DO.Dependency(FindDependent(item.Id, dependency.Id), item.Id, dependency.Id));
                }
            }
